...
class AdminController extends Controller
{
    ...
    {
		// 年度がセレクトボックスで選択されていたらyearはその値になる
		$year = 0;
        if(!empty($_GET["year"])){
            $year = $_GET['year'];
        }else{
            $year = $this->getYearMax();
        }
        ...
        // 年度の一覧を取得
        $year_list = $this->getYearList($year);
        
        return view('admin.venue')->with(compact('ques', 'answer_count', 'year_list'));
    }
	...
	*/
    public function edit()
    {
        ...
        // セレクトボックスから選択された場合は、GETで取得した値を表示する
        $year = 0;
        if(!empty($_GET["year"])){
            $year = $_GET['year'];
        }else{
    ...
    */
    ...
    /*
    	return 配列
        view側でのセレクトボックス用
        一番過去のものを2015年とし、今年までの年を配列に入れ込んでいく
    */
    private function getYearList($year) {
		$year = $this->getYearMax();
	...
	// year_listで表示する最大年度を返す
	private function getYearMax() {
		return (int)date('Y') + 1;
	}
}
